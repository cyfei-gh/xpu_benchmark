...
class GemmBenchmark:
    ...
    def run(
        ...
        print(f"{'='*100}")
        print(f"{'M':>6} {'N':>6} {'K':>6} | {'dtype':<13} | "
              f"{'time(ms)':>12} | {'TFLOPS':>8} | {'theo_TFLOPS':>9} | {'MFU':>7} | "
              f"{'BW(GB/s)':>10} | {'MBU':>8}")
        print(f"{'-'*120}")
...
class LLMGemmBenchmark:
    ...
    Example usage:
        ...
        bench.plot_batch_tflops_curve(results, 'llm_gemm_curves.png')
    ...
    def run(
        ...
        Args:
            model_name: Key in MODEL_SHAPE dict (e.g. 'HY-image-3.0', 'DeepSeek-V2').
        ...
        for dtype_str in dtypes:
            ...
            for batch_size in batch_sizes:
                for wl_name, (k_base, n_base), split_dim in shape_list:
                    ...
                    gemm_result = self._gemm_bench.run_single(m, n, k, dtype_str)
